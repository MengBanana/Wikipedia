  ...
  Meteor.methods({
    "getData"(search) {
      ...
      let res = Promise.all(colors.map(c => (
        axios
          .get(`https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=491c766aeebbdf4c9c3b276591632e2c&text=${search.search}+${c}&sort=relevance&format=json&nojsoncallback=1`)
          .then(data => data.data))));
      console.log(res);
      return res;
    }
